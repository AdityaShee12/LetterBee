...
const GroupChatPage = () => {
  ...
  const { groupId, groupName, groupAvatar, groupAbout, groupMembers } =
  ...
  useEffect(() => {
    socket.emit("groupClick", { groupMembers, groupId });

    socket.on("onlineMember", (onlineMember) => {
      if (onlineMember) {
        setOnlineMember(onlineMember);
        let i;
        for (let j = 0; j < onlineMember.length; j++) {
          i = i + 1;
        }
        setOnlineCount(i);
      }
...
>>>>>>>> 443d633a0449f716b0c49857be001897e1d6840e:LetterBee-Frontend/src/services/chat.service.jsx
  ...
  const sendMessage = async () => {
    ...
    if (state === "online") {
      socket.emit("send groupMessage", sendItem);
      setMessages((prev) => [
        ...prev,
